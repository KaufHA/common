...
#ifdef USE_ESP8266
  this->pref_ = global_preferences->make_preference<wifi::SavedWifiSettings>(hash, true, this->forced_addr);
#else
...
void WiFiComponent::setup_ap_config_() {
  ...
  if (this->ap_.ssid_.empty()) {
    ...
    // KAUF: append version suffix (e.g. " v2.10") to the SSID, truncating hostname if needed
#ifdef ESPHOME_PROJECT_VERSION
    {
      // Build version suffix
      char ver_suffix[8];
      const char *ver = ESPHOME_PROJECT_VERSION;
      size_t ver_len = strlen(ver);
      if (ver_len > 5)
        ver_len = 5;
      ver_suffix[0] = ' ';
      ver_suffix[1] = 'v';
      memcpy(ver_suffix + 2, ver, ver_len);
      size_t ver_suffix_len = 2 + ver_len;
      // Remove trailing '(' to limit to x.xx
      if (ver_suffix[ver_suffix_len - 1] == '(')
        ver_suffix_len--;
      ver_suffix[ver_suffix_len] = '\0';

      // Re-read current SSID and append version, truncating hostname part if needed
      const auto cur_ssid = this->ap_.get_ssid();
      size_t cur_len = cur_ssid.size();
      size_t total_len = cur_len + ver_suffix_len;

      char ssid_buf[AP_SSID_MAX_LEN + 1];
      if (total_len <= AP_SSID_MAX_LEN) {
        memcpy(ssid_buf, cur_ssid.c_str(), cur_len);
        memcpy(ssid_buf + cur_len, ver_suffix, ver_suffix_len);
        ssid_buf[total_len] = '\0';
      } else {
        // Truncate the hostname portion to make room for version suffix
        // Keep MAC suffix (last 7 chars) if present, plus version suffix at the end
        size_t mac_suffix_len = App.is_name_add_mac_suffix_enabled() ? AP_SSID_SUFFIX_LEN : 0;
        size_t keep_at_end = mac_suffix_len + ver_suffix_len;
        size_t hostname_space = AP_SSID_MAX_LEN - keep_at_end;
        size_t pos = 0;
        memcpy(ssid_buf, cur_ssid.c_str(), hostname_space);
        pos = hostname_space;
        if (mac_suffix_len > 0) {
          memcpy(ssid_buf + pos, cur_ssid.c_str() + cur_len - mac_suffix_len, mac_suffix_len);
          pos += mac_suffix_len;
        }
        memcpy(ssid_buf + pos, ver_suffix, ver_suffix_len);
        pos += ver_suffix_len;
        ssid_buf[pos] = '\0';
      }
      this->ap_.set_ssid(ssid_buf);
...
WiFiRetryPhase WiFiComponent::determine_next_phase_() {
  switch (this->retry_phase_) {
    ...
    case WiFiRetryPhase::FAST_CONNECT_CYCLING_APS:
      ...
      // KAUF: allow disable fast connect
