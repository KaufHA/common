...
void ESP8266PWM::dump_config() {
  ...
  if (this->align_pin_ >= 0) {
    ESP_LOGCONFIG(TAG, "  Align pin: GPIO%d  Phase offset: %.2f", this->align_pin_, this->phase_offset_);
  }
#endif
...
#ifdef KAUF_ESP8266_PHASE_LOCKED_PWM
    if (this->align_pin_ >= 0) {
      auto offset_us = static_cast<uint32_t>(roundf(this->phase_offset_ * total_time_us));
